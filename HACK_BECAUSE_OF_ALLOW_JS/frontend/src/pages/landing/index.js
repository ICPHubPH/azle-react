...
import { dummyFeedbacks, dummyPosts, dummyTopProviders, dummyUsers, } from "./dummy-data";
    ...
    return (<div className="scroll-smooth">
      ...
      <div className="flex flex-col gap-4 items-center container mx-auto">
        ...
        <div className="px-4 grid grid-cols-1 gap-4 md:grid-cols-2 lg:gap-8 lg:grid-cols-3">
          {dummyPosts.map((post) => (<PostSummaryCard postId={post.id} postAuthorEmail={post.email} postAuthorAvatarSource={post.avatarSource} postTitle={post.postTitle} postThumbnailSource={post.postThumbnailSource} postDescription={post.postDescription} postRatingCount={post.postRatingCount} postBookmarkCount={post.postBookmarkCount} postAuthorName={post.name} postCommentCount={post.postCommentCount} postType={post.postType} postDate={post.postDate}/>))}
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4 items-center container mx-auto mt-6">
        ...
        <div className="w-full px-4 grid grid-cols-1 gap-4 md:grid-cols-2 lg:gap-8 lg:grid-cols-3">
          {dummyTopProviders.map((provider, index) => (<TopProviderCard key={index} thumbnail={provider.thumbnail} avatar={provider.avatar} provider={provider.provider} description={provider.description} scholarship={provider.scholarship} id={""}/>))}
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4 items-center container mx-auto mt-6">
        ...
        <div className="w-full px-4 grid grid-cols-1 gap-4 md:grid-cols-2 lg:gap-8 lg:grid-cols-3">
          {dummyFeedbacks.map((feedback, index) => (<Feedback user={dummyUsers[index]} feedback={feedback}/>))}
        </div>
      </div>
