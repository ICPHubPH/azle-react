...
export default function ProvidersFeed() {
    ...
    const filteredProviders = dummyTopProviders
        ...
        .filter((provider) => provider.provider.toLowerCase().includes(searchQuery.toLowerCase()) ||
        ...
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value)}>
          ...
          <div className="flex space-x-4 mb-6">
            <div className="relative flex-grow">
              ...
              <Input placeholder="Search providers..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-8"/>
            </div>
          ...
          <TabsContent value="all" className="mt-0">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredProviders.map((provider) => (<TopProviderCard key={provider.id} thumbnail={provider.thumbnail} avatar={provider.avatar} provider={provider.provider} description={provider.description} scholarship={provider.scholarship} id={provider.id}/>))}
            </div>
          </TabsContent>
          ...
          <TabsContent value="school" className="mt-0">
            ...
            .map((provider) => (<TopProviderCard key={provider.id} thumbnail={provider.thumbnail} avatar={provider.avatar} provider={provider.provider} description={provider.description} scholarship={provider.scholarship} id={provider.id}/>))}
            </div>
          </TabsContent>
