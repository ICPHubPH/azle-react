...
export default function ProviderProfile() {
    ...
    return (<div className="container mx-auto px-4 py-8 max-w-2xl">
      <Button variant="ghost" onClick={() => navigate(-1)} className="mb-6 p-0 hover:bg-transparent dark:hover:bg-transparent">
        ...
        <CardHeader className="pb-0">
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
            ...
            <div className="text-center sm:text-left">
              ...
              <CardTitle className="text-2xl sm:text-3xl font-bold mb-2">{provider.provider}</CardTitle>
              <div className="flex items-center justify-center sm:justify-start text-muted-foreground mb-2">
                <MapPin className="h-4 w-4 mr-1"/>
                <span className="text-sm">{provider.location}</span>
              </div>
              <div className="flex items-center">
                {[...Array(5)].map((_, i) => (<Star key={i} className={`h-4 w-4 ${i < provider.rating ? 'text-yellow-400' : 'text-gray-300 dark:text-gray-600'}`} fill="currentColor"/>))}
        ...
        <CardContent className="mt-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">{provider.scholarship}</p>
            </div>
            <div>
          ...
          <div className="flex flex-wrap gap-2">
            {['Online', 'In-Person', 'Flexible Schedule', 'Certification'].map((tag) => (<Badge key={tag} variant="secondary">
                {tag}
              </Badge>))}
          </div>
        </CardContent>
