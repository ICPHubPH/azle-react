    ...
    const NavItems = () => (<>
      {navItems.map((item, index) => (<li key={index}>
          <Button variant="ghost" asChild className="w-full justify-start hover:bg-transparent hover:text-blue-500 transition-colors duration-200 dark:hover:text-yellow-500">
            <a onClick={() => setIsMobileMenuOpen(false)} className="hover:underline hover:text-blue-500 dark:hover:text-yellow-500 transition-colors duration-200 cursor-pointer">
              {item.name}
            </a>
          </Button>
        </li>))}
          ...
          <SheetContent side="right" className="w-[250px] sm:w-[300px]">
            ...
            <div className="border-t mt-4 pt-4">
              {user ? (<>
                  ...
                  <Button variant="ghost" className="w-full justify-start mt-2" onClick={() => navigate('') /* Implement logout logic */}>
                    Logout
                  </Button>
