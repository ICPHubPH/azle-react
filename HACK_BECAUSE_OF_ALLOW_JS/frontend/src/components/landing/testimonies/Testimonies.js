...
const Testimonies = () => {
    return (<section className="pt-20 px-10 ">
      <h2 className="text-3xl font-bold text-center mb-6 text-blue-600 dark:text-yellow-500">Student Testimonials</h2>
      <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-3 gap-8">
        {testimonies.map((testimony, index) => (<TestimonyCard key={index} avatar={testimony.avatar} name={testimony.name} testimony={testimony.testimony}/>))}
      </div>
    </section>);
};
