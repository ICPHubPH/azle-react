...
export default function PostSection() {
    ...
    return (<div className="min-h-96 w-full h-full border px-8 py-4 rounded-lg">
...
function PostFrontGroup({ post }) {
    return (<div className="w-full h-full relative border-2 rounded-t-lg">
      <img src={post.postThumbnailSource} alt={post.postTitle} className="object-cover w-full h-full aspect-video rounded-t-md"/>
      <Button variant={null} size={null} className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent hover:backdrop-blur-sm transition-all ease-in-out duration-100" onClick={(e) => {
            e.preventDefault();
            e.stopPropagation();
...
function MetaDataGroup({ post }) {
    return (<div className="w-full flex justify-between items-center py-4 px-8 bg-gradient-to-r from-blue-800 to-purple-800">
      ...
      <div className="flex flex-col gap-1 items-end justify-start">
        ...
        <Button variant={"default"} className="flex gap-2 font-normal bg-blue-600 text-white">
