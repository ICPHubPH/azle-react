...
const Header = () => {
    ...
    return (<header className="border-b z-50 sticky top-0 backdrop-blur-md">
      <div className="container mx-auto flex justify-between items-center h-16 px-4">
        <div className="flex items-center space-x-2">
          <h1 className="text-2xl font-bold bg-gradient-to-r from-[#0038a9] via-[#ce1127] to-[#f5ce31] bg-clip-text text-transparent cursor-pointer" onClick={handleLogoClick} // Added onClick for the logo
    >
            ConnectED
          </h1>
          {/* Desktop Menu */}

          {!isMobile && (<ul className="hidden md:flex space-x-4">
              <NavItems />
            </ul>)}
        </div>
        ...
        <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
          ...
          <SheetContent side="right" className="w-[250px] sm:w-[300px]">
            ...
            <div className="border-t mt-4 pt-4">
              <Button variant="ghost" className="w-full justify-start" onClick={() => navigate('/profile')} // Replace with actual profile path
    >
                Profile
              </Button>
              <Button variant="ghost" className="w-full justify-start mt-2" onClick={() => navigate('/logout')} // Replace with actual logout path
    >
                Logout
              </Button>
            </div>
